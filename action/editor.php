...
class action_plugin_bpmnio_editor extends ActionPlugin
{
    ...
    public function handleForm(Event $event)
    {
        ...
        $this->loadPngCache();
        $cacheKey = plugin_bpmnio_png_cache::buildKey($type, trim((string) $TEXT), '', (string) $ID);
        ...
        $form->addHTML(<<<HTML
            <div class="plugin-bpmnio" id="plugin_bpmnio__{$type}_editor">
                ...
                <div class="{$type}_js_canvas">
                    <div class="{$type}_js_container" data-png-cache-key="{$cacheKey}" {$lintAttr}></div>
                </div>
            </div>
            HTML);
    ...
    public function handlePngCacheAjax(Event $event): void
    {
        ...
        // CSRF: require a valid DokuWiki security token.
        if (!checkSecurityToken($INPUT->post->str('sectok'))) {
            $this->sendJsonResponse(403, ['ok' => false, 'error' => 'invalid-token']);
            return;
        }

        // ACL: only users who could edit the page may seed its PNG cache,
        // since the cached image is a derivative of the page's authored XML.
        $id = cleanID($INPUT->post->str('id'));
        if ($id === '' || auth_quickaclcheck($id) < AUTH_EDIT) {
            $this->sendJsonResponse(403, ['ok' => false, 'error' => 'forbidden']);
            return;
        }
        ...
        $ok = plugin_bpmnio_png_cache::save($key, $png, $id);
